...
//	}
	...
	public static BswabeCph enc(String []u,BswabePub pub, String []policy, Index index)
		...
		// 将 t1 和 t2 设置为基于 index 的固定值
		Element t1 = pairing.getZr().newElement().setFromHash(index.word.getBytes(), 0, index.word.getBytes().length);
		Element t2 = pairing.getZr().newElement().setFromHash(index.word.getBytes(), 0, index.word.getBytes().length);
		Element m=pairing.getZr().newElement();//要加密的元素,将字符串映射到z上
